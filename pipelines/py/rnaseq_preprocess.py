...
def fetch_gene_info(
    ...
    """
    # Set output_db to a list of default values
    # IDE complains about default value being mutable
...
def create_counts_matrix(context_name):
    ...
    matrix_output_dir = os.path.join(
        configs.rootdir, "data", "data_matrices", context_name
    )
...
def create_config_df(context_name):
    ...
    for gcfilename in gene_counts_files:
        try:
            label = re.findall(
                r"S[1-9][0-9]?[0-9]?R[1-9][0-9]?[0-9]?r?[1-9]?[0-9]?[0-9]?", gcfilename
            )[0]
        ...
        context_path = os.path.join(
            configs.rootdir, "data", "MADRID_input", context_name
        )
...
def create_gene_info_file(matrix_file_list, form, taxon_id):
    ...
    gene_info = fetch_gene_info(
        genes, input_db=form, output_db=output_db, taxon_id=taxon_id
    )
...
def handle_context_batch(context_names, mode, form, taxon_id, provided_matrix_file):
    ...
    trnaseq_config_filename = os.path.join(
        configs.rootdir, "data", "config_sheets", "trnaseq_data_inputs_auto.xlsx"
    )
    mrnaseq_config_filename = os.path.join(
        configs.rootdir, "data", "config_sheets", "mrnaseq_data_inputs_auto.xlsx"
    )
...
def main(argv):
    ...
    context_names = (
        context_names.strip("[").strip("]").replace("'", "").replace(" ", "").split(",")
    )  # convert to py list

    if gene_format.upper() in [
        "ENSEMBL",
        "ENSEMBLE",
        "ENSG",
        "ENSMUSG",
...
if __name__ == "__main__":
    ...
    f = open(
        os.path.join(configs.rootdir, "py", "rscripts", "generate_counts_matrix.R"), "r"
    )
    string = f.read()
    f.close()
