...
def load_proteomics_data(datafilename, context_name):
    ...
    dataFullPath = os.path.join(
        configs.root_dir, "data", "data_matrices", context_name, datafilename
    )
...
def abundance_to_bool_group(
    ...
    output_dir = os.path.join(
        configs.root_dir, "data", "results", context_name, "proteomics"
    )
    ...
    # Z-tranform
    rpy2_api.Rpy2(
        r_file_path, abundance_filepath, output_dir, group_name
    ...
    bool_filepath = os.path.join(
        output_dir, f"bool_prot_Matrix_{context_name}_{group_name}.csv"
    )
...
def to_bool_context(context_name, group_ratio, hi_group_ratio, group_names):
    ...
    for group in group_names:
        read_filepath = os.path.join(
            output_dir, f"bool_prot_Matrix_{context_name}_{group}.csv"
        )
        ...
        if not merged_df.empty:
            merged_df = pd.merge(
                merged_df, read_df["expressed"], right_index=True, left_index=True
            )
            merged_hi_df = pd.merge(
                merged_hi_df, read_df["high"], right_index=True, left_index=True
            )
...
def main(argv):
    ...
    for context_name in sheet_names:
        ...
        for group in groups:
            ...
            ).ravel().tolist() + ["Gene Symbol", "uniprot"]
            cols = np.take(
                config_sheet["SampleName"].to_numpy(), group_idx
