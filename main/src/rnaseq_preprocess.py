...
configs = Configs()
r_file_path: Path = Path(
    configs.root_dir, "src", "rscripts", "generate_counts_matrix.R"
)
...
def create_counts_matrix(context_name):
    ...
    matrix_output_dir = os.path.join(
        configs.root_dir, "data", "data_matrices", context_name
    )
    ...
    rpy2_hook = rpy2_api.Rpy2(
        r_file_path=r_file_path, data_dir=input_dir, out_dir=matrix_output_dir
    )
...
def create_config_df(context_name):
    ...
    """
    gene_counts_glob = os.path.join(
        configs.root_dir, "data", "COMO_input", context_name, "geneCounts", "*", "*.tab"
    )
    gene_counts_files = glob.glob(gene_counts_glob, recursive=True)

    out_df = pd.DataFrame(
        columns=["SampleName", "FragmentLength", "Layout", "Strand", "Group"]
    )
    ...
    for gcfilename in gene_counts_files:
        ...
        context_path = os.path.join(
            configs.root_dir, "data", "COMO_input", context_name
        )
        ...
        if len(prep_glob) < 1:
            print(
                f"No prep file found for {label}, assuming 'total' as in Total RNA library preparation"
            )
            prep = "total"
            ...
            with open(prep_glob[0]) as file:
                ...
                if prep not in ["total", "mrna"]:
                    print(
                        "prep_method.txt must have either 'mrna' or 'total', or be absent to assume 'total'."
                    )
                    sys.exit()
        ...
        if len(frag_glob) < 1:
            ...
            if layout == "single-end":
                ...
                if not multi_flag:
                    ...
                    mean_fragment_size = sum(
                        frag_df["meanxcount"] / sum(frag_df["frag_count"])
                    )
                    ...
                    for ff in frag_files:  # type: ignore
                        ...
                        frag_df["meanxcount"] = (
                            frag_df["frag_mean"] * frag_df["frag_count"]
                        )
                        mean_fragment_size = sum(
                            frag_df["meanxcount"] / sum(frag_df["frag_count"])
                        )
                        mean_fragment_sizes = np.append(
                            mean_fragment_sizes, mean_fragment_size
                        )
                        library_sizes = np.append(
                            library_sizes, sum(frag_df["frag_count"])
                        )

                    mean_fragment_size = sum(mean_fragment_sizes * library_sizes) / sum(
                        library_sizes
                    )
...
def split_counts_matrices(count_matrix_all, df_total, df_mrna):
    ...
    matrix_total = matrix_all[
        ["genes"]
        + [n for n in matrix_all.columns if n in df_total["SampleName"].tolist()]
    ]
    matrix_mrna = matrix_all[
        ["genes"]
        + [n for n in matrix_all.columns if n in df_mrna["SampleName"].tolist()]
    ]
...
def create_gene_info_file(
    ...
    gene_info["end_position"] = gene_info["Chromosomal Location"].str.extract(
        r"chr_end: (\d+)"
    )
    gene_info.rename(
        columns={"Gene Symbol": "hgnc_symbol", "Gene ID": "entrezgene_id"}, inplace=True
    )
...
def handle_context_batch(
    ...
    """
    trnaseq_config_filename = os.path.join(
        configs.root_dir, "data", "config_sheets", "trnaseq_data_inputs_auto.xlsx"
    )
    mrnaseq_config_filename = os.path.join(
        configs.root_dir, "data", "config_sheets", "mrnaseq_data_inputs_auto.xlsx"
    )
    ...
    for context_name in context_names:
        ...
        gene_output_dir = os.path.join(
            configs.root_dir, "data", "results", context_name
        )
        matrix_output_dir = os.path.join(
            configs.root_dir, "data", "data_matrices", context_name
        )
        ...
        matrix_path_all = os.path.join(
            matrix_output_dir, ("gene_counts_matrix_full_" + context_name + ".csv")
        )
        matrix_path_total = os.path.join(
            matrix_output_dir, ("gene_counts_matrix_total_" + context_name + ".csv")
        )
        matrix_path_mrna = os.path.join(
            matrix_output_dir, ("gene_counts_matrix_mrna_" + context_name + ".csv")
        )
