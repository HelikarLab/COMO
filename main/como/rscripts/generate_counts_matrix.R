...
organize_gene_counts_files <- function(data_dir) {
  ...
  for (j in 1:length(count_dir)) {
    ...
    replicate_names <- rep("", n_replicates) # initialize empty array to store rep names
    strandedness_files <- rep("", n_replicates) # initialize empty array to store

    for (i in 1:n_replicates) { # for each replicate, get name of replicate ex. S1R1
      replicate_file <- stringr::str_match(counts_files[i], "geneCounts/\\s*(.*?)\\s*.tab")[, 2] # file path
      cat(paste0("replicate_file=", replicate_file, "\n"))

      replicate_names[i] <- basename(replicate_file) # file name only
      cat(paste0("replicate_name=", replicate_names[i], "\n"))

      rname <- sub(pattern = "(.*)\\..*$", replacement = "\\1", basename(replicate_file)) # remove file extension
      strandedness_file <- paste0(rname, "_strandedness.txt") # use rep name to get matching strand
      cat(paste0("strandedness_file=", strandedness_file, "\n"))
...
if (sys.nframe() == 0) {  # Run main function if using Rscript
  args <- commandArgs(trailingOnly = TRUE)
  if (length(args) != 2) {
    stop("Usage: Rscript generate_counts_matrix.R 'data_dir' 'out_dir'")
  }
  generate_counts_matrix_main(args[1], args[2])
}