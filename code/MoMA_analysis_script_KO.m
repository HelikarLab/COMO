...
% This is the update we made in results using both WT moma and Del moma
fluxSolutionRatios=[];
for i= 1:size(hasEffect_DTgenes)
	modelWT=model;
	modelDel=deleteModelGenes(model,hasEffect_DTgenes{i});
 
	[solutionDel, solutionWT] = MOMA(modelWT, modelDel,'max');

	FSratios=solutionDel.x./solutionWT.x;
...
% Run this twice for UP and Down 

fid = fopen('MS_UP.txt'); % change filename, run this for both up and down regulated genes
...
writetable(T,'Gene_Pairs_Inhi_Fratio_UP.txt')
